...
class E2EEStripView: UIView {
    ...
    private func buildMainView() {
        ...
        // VStack: info, input, icons, encoding label
        let vStack = UIStackView(arrangedSubviews: [infoLabel, inputField, iconStack, encodingLabel])
...
"""
    ...
    /// Icon 2 - Lock: smart action from clipboard
    /// - If clipboard has encrypted message from known contact -> decrypt + show in history
    /// - If clipboard has invite (PreKeyResponse) -> show add contact
    /// - If clipboard empty -> flash info
    @objc private func lockTapped() {
        deactivateAllInput()
        flashInfo("Reading clipboard...")
    ...
    private func processClipboard() {
        ...
        guard let clipText = UIPasteboard.general.string, !clipText.isEmpty else {
            Logger.log("[LOCK] Clipboard is empty or nil")
            flashInfo("Clipboard empty - copy a message first")
            return
        }
        Logger.log("[LOCK] Raw clipboard length=\(clipText.count) chars")
        Logger.log("[LOCK] Raw clipboard first 300 chars: \(String(clipText.prefix(300)))")
        ...
        do {
            ...
            flashInfo("Not a valid encrypted message")
            return
        }
        ...
        if envelope.signalProtocolAddressName == SignalProtocolManager.shared.accountName {
            Logger.log("[LOCK] This is our own message, skipping")
            flashInfo("Cannot decrypt own messages")
        ...
        case .signalMessage:
            ...
            if let sender = sender {
                ...
                flashInfo("Unknown sender -- try adding as contact first")
            }
